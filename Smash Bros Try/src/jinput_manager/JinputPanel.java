...
public class JinputPanel extends JPanel implements PlayerListener{
	...
	public JinputPanel(int num_of_players){
		super();
	...
	public void switchPlayer(JinputPlayer player){
		HashMap<Component,String> bgs = player.bindings;
		...
		if(player.controller != null){
			...
			while(i.hasNext()){
				...
				sets.get(pairs.getValue()).setComponent(pairs.getKey().getName());
			}
	...
	public void setInputs(int playernumber, HashMap<String, Float> inputs) {
		if(playernumber == currentPlayer.playernumber){
			...
			while(i.hasNext()){
				Map.Entry<String, Float> pairs = (Map.Entry)i.next();
				if(sets.get(pairs.getKey()).isSet()){
					if(pairs.getValue() > 0.5f) sets.get(pairs.getKey()).toggle(true);
					else  sets.get(pairs.getKey()).toggle(false);
				}
			}
		}
	}
	...
	public void push(int playernumber, Object o) {
		if(playernumber == currentPlayer.playernumber){
			...
			if(waiting != 0L && System.currentTimeMillis() - waiting < 5000 && currentPlayer.bindings.get(temp) == null){
				if(!temp.isAnalog() && temp.getPollData() > 0.5f){
					bind(temp, butear);
					butear = null;
					waiting = 0L;
				}
	...
	public void bind(Component com, String setting){
		currentPlayer.bind(com, setting);
		sets.get(setting)
		.setComponent(com.getName());
	}
